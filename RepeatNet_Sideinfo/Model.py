...
class RepeatNet(nn.Module):
    def __init__(self, embedding_size, hidden_size, item_vocab_size, side_size):
        """
        ...
        self.explore_feature_raw_trans = nn.Linear(4*hidden_size, 2*hidden_size)
        ...
        self.mode_attn = BilinearAttention(2*hidden_size, 2*hidden_size, 2*hidden_size)
        self.mode = nn.Linear(2*hidden_size, 2)

        self.repeat_attn = BilinearAttention(2*hidden_size, 2*hidden_size, 2*hidden_size)
        self.explore_attn = BilinearAttention(2*hidden_size, 2*hidden_size, 2*hidden_size)
        self.explore = nn.Linear(2*hidden_size, item_vocab_size)
    ...
    def model(self, data):
        ...
        #print(explore_feature.squeeze(1).shape)
        explore_feature_transformed = self.explore_feature_raw_trans(torch.cat([state, explore_feature.squeeze(1)], dim=1))
