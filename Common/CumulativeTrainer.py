...
class CumulativeTrainer(object):
    ...
    def train_epoch(self, method, train_dataset, train_collate_fn, batch_size, epoch, optimizer, scheduler=None):
        ...
        for j, data in enumerate(tqdm(train_loader), 0):
            if torch.cuda.is_available():
                data_cuda = dict()
    ...
    def predict(self, method, dataset, collate_fn, batch_size, epoch, output_path, mode='valid'):
                ...
                if torch.cuda.is_available():
                    ...
                    for key, value in data.items():
                        if isinstance(value, torch.Tensor):
                            ...
                            data_cuda[key] = value
                        #data_cuda[key] = value # only use cpu, instead of gpu
                    data = data_cuda
